...
export function OpportunityCTA() {
  return (
    <section className="py-28 bg-gradient-to-b from-white via-neutral-50/50 to-white overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <motion.div
              ...
              viewport={{ once: true }}
            >
              <span className="inline-block px-4 py-1.5 rounded-full bg-blue-50 text-blue-700 text-sm font-semibold mb-6 border border-blue-100">
              ...
              <h2 className="text-4xl md:text-5xl font-bold text-neutral-900 mb-4 tracking-tight leading-[1.1]">
                ...
                <span className="bg-gradient-to-r from-blue-600 to-violet-600 bg-clip-text text-transparent">Starts Here</span>
              </h2>
            </motion.div>
            ...
            <motion.p
              ...
              className="text-lg text-neutral-500 mb-8 leading-relaxed"
            >
            ...
            >
              <Link
                href="/opportunity"
                className="group inline-flex items-center justify-center gap-2 rounded-full bg-neutral-900 px-8 py-4 text-white font-semibold hover:bg-neutral-800 transition-all duration-200 hover:shadow-lg"
              >
              ...
              <Link
                href="/user/dashboard"
                className="inline-flex items-center justify-center gap-2 rounded-full border border-neutral-200 px-8 py-4 text-neutral-700 font-semibold hover:border-neutral-300 hover:bg-neutral-50 transition-all duration-200"
              >
                Log In to Dashboard
              </Link>
            </motion.div>
          ...
          >
            <div className="relative">
              {/* Role cards */}
              <div className="bg-white rounded-3xl border border-neutral-100 shadow-xl shadow-neutral-200/50 p-8 space-y-6">
                <h3 className="text-lg font-bold text-neutral-900">Who is this for?</h3>
                <div className="flex flex-wrap gap-3">
                  {roles.map((r) => (
                    <div key={r.label} className="flex items-center gap-2.5 px-4 py-2.5 rounded-2xl bg-neutral-50 border border-neutral-100 hover:border-neutral-200 transition-colors">
                ...
                <div className="grid grid-cols-3 gap-4">
                  {stats.map((s) => (
                    <div key={s.label} className="text-center">
                      <span className="text-2xl font-bold text-neutral-900 tracking-tight">{s.value}</span>
                      <span className="block text-xs text-neutral-400 mt-1">{s.label}</span>
                    </div>
                  ))}
                </div>
              ...
              {/* Decorative */}
              <div className="absolute -z-10 -top-3 -right-3 w-full h-full rounded-3xl bg-gradient-to-br from-blue-50 to-violet-50 border border-blue-100/50" />
            </div>
          </motion.div>
        </div>
      </div>
